...
void solve()
{
    ...
    FOR(i, 31)
    {
        // we need to set all the (31-i-1)th bit of all the elements for it to be considered in the final answer
        ll req = n - bits[i];
        // we can set this bit of all the elements of our array thus considering this in our answer
        if (req <= k)
        {
            ans |= 1 << (31 - i - 1), k -= req;
        }
