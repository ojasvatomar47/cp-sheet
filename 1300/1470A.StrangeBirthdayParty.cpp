...
void solve()
{
    // greedy: have two options: whether select the current cost or the min cost of gift which hasn't been bought yet
    ll n, m;
    cin >> n >> m;
    VEC v1(n), v2(m);
    FOR(i, n)
    ...
    {
        // x: v1[i]th item cost 
        // y: min. cost of price not taken yet
        // if(x==y) we'll prefer x
        ll x = v2[v1[i] - 1], y = v2[j];
        if (y < x)
            j++, ans += y;
        else
            ans += x;
