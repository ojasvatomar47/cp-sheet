...
{
    // 2 array greedy
    // need to find out the min distance of each one from first and last endpoints -> apply 2 array greedy
    ll n, k, sum = 0;
    cin >> n >> k;
    VEC v(n), v1, v2;
    FOR(i, n)
    {
        cin >> v[i];
        if (v[i] == 1)
            v1.push_back(i + 1), v2.push_back(n - i);
        sum += v[i];
    }
    ...
    ll mini = min(v1[diff - 1], v2[diff - 1]), ind = diff - 2;

    FOR(i, diff - 1)
    {
        mini = min(mini, v1[ind--] + v2[i]);
    }
    print(mini);
}
