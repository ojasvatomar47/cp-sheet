...
void solve()
{
    // k bits in each element set (for max. sum)
    // for AND = 0 atlease one element should have one bit as zero for each k bits
    // n elements -> select one element -> make one of it's bit zero -> do this for k times
    // nC1 * nC1 * nC1 ... (k times)
    ll n, k, prod = 1;
    cin >> n >> k;
    for (int i = 0; i < k; i++)
