...
void solve()
{
    ...
    FOR(i, n / 2)
    {
        FOR(j, n / 2)
        {
            ll ones = v[i][j] + v[j][n - i - 1] + v[n - j - 1][i] + v[n - i - 1][n - j - 1], zeroes = 4 - ones;
            // deb(ones, zeroes);
            ans += 4 - max(ones, zeroes);
        }
    }
    ...
    if (n & 1)
    {
        FOR(i, n / 2)
        {
            ll ones = v[i][n / 2] + v[n - i - 1][n / 2] + v[n / 2][i] + v[n / 2][n - i - 1], zeroes = 4 - ones;
            // deb(ones, zeroes);
            ans += 4 - max(ones, zeroes);
        }
    }
    ...
    //     FOR(j, n / 2)
    //     {
    //         ll ones = v[i][j] + v[j][n - i - 1] + v[n - i - 1][n - j - 1] + v[n - j - 1][i], zeroes = 4 - ones;
    //         // deb(ones, zeroes);
    //         ans += 4 - max(ones, zeroes);
    //     }
    // }
