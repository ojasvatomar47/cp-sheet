...
void solve()
{
    // just need to calclulate the number of times B takes an extra step because of A being on the same cell (ie, n/2)
    ll n, k, ans;
    cin >> n >> k;
    if (n % 2 == 0)
    {
    ...
    {
        // k--;
        // on every n/2 th step, B increases it's position by 1 extra
        ll f = n / 2;
        // no. of such steps in k time B would take
        ll x = (k - 1) / f;
