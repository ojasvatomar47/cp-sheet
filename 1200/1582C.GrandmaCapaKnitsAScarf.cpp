...
void solve()
{
    // since we can only delete the occurences of one character tells us in itself to use brute force
    ll n, mini = INT_MAX;
    cin >> n;
    string str;
    cin >> str;
    ...
    {
        ll i = 0, j = n - 1, count = 0, flag = 0;

        // check number of occurences of ch to delete for making the string palindrome
        while (i < j)
        {
            if (str[i] == str[j])
                i++, j--;
